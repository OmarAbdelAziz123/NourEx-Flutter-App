...
class ProductsRepos {
  ...
  /// Make Review
  Future<ApiResult<String>> makeReview({required String productId, required String comment, required int rating}) async {
    try {
      final response = await productsApiServices.makeReview(productId: productId, comment: comment, rating: rating);
      if(response == null) {
        return ApiResult.failure(ErrorHandler.handleApiError(null));
      }
